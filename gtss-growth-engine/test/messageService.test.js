...
test("Message generation template resolution and character limit enforcement", async () => {
  ...
  assert.ok(
    resultLinkedIn.variantA.body.includes("Hi Alice"),
    "LinkedIn message must contain the first name in the salutation",
  );
  assert.equal(
    resultLinkedIn.variantA.body.includes("Alice Smith"),
    false,
    "LinkedIn message must not use the full name in the salutation",
  );
  ...
  assert.ok(xBody.includes("Hi Bob"), "X message must contain the first name in the salutation");
  assert.equal(xBody.includes("Bob Jones"), false, "X message must not use the full name in the salutation");
  ...
  assert.equal(
    xBody.includes("Would love to connect!"),
    false,
    "X message must NOT contain LinkedIn connect assumption",
  );
