...
const db = getDb();
try {
  const connCols = db.prepare("PRAGMA table_info(connection_jobs)").all().map(c => c.name);
  if (!connCols.includes("retry_count")) {
    db.exec("ALTER TABLE connection_jobs ADD COLUMN retry_count INTEGER DEFAULT 0");
  }
  if (!connCols.includes("next_retry_at")) {
    db.exec("ALTER TABLE connection_jobs ADD COLUMN next_retry_at TEXT");
  }
} catch (err) {
  console.error("[CONNECTION-QUEUE] Defensively handled schema migration error on startup:", err.message);
}
...
function promoteRelatedDmJob(txDb, job, policy, forceImmediate = false) {
  ...
  const isImmediate = ["x", "instagram", "facebook"].includes(normPlatform) || forceImmediate;

  if (isImmediate) {
    const minDelaySeconds = policy.delays?.actionMinSeconds || 30;
    const scheduledAt = new Date(Date.now() + minDelaySeconds * 1000).toISOString();
    ...
    if (res.changes > 0) {
      ...
      queueLog("info", "connection_queue", job.id, `Promoted related pending DM job for lead ${job.lead_id} (Scheduled at: ${scheduledAt}).`);
    }
  }
}
...
async function processConnectionQueue(page, options = {}) {
  ...
  queueLog("info", "connection_queue", "SYSTEM", "Executing connection queue processing loop...");
  ...
  try {
    ...
    queueLog("error", "connection_queue", "SYSTEM", `Failed to query eligible jobs: ${err.message}`);
    return report;
  }

  if (eligibleJobs.length === 0) {
    queueLog("info", "connection_queue", "SYSTEM", "No eligible connection jobs found.");
    return report;
  }

  queueLog("info", "connection_queue", "SYSTEM", `Found ${eligibleJobs.length} eligible connection jobs for active campaigns.`);
  ...
  for (const job of eligibleJobs) {
    ...
    try {
      ...
      if (!policy || !platformLimits) {
        queueLog("warn", "connection_queue", job.id, `Skipping job due to unsupported platform configurations: ${job.platform}`);
        continue;
      }
      ...
      if (expiredPlatforms.has(normPlatform)) {
        try {
          ...
          queueLog("error", "connection_queue", job.id, `Failed to postpone job after session expiration skip: ${err.message}`);
        }
        queueLog("info", "connection_queue", job.id, `Postponed job: Platform '${normPlatform}' session is already known to be expired in this batch.`);
      ...
      try {
        ...
        queueLog("error", "connection_queue", job.id, `Failed to query campaign status: ${err.message}`);
        continue;
      }

      if (!campaignRow || campaignRow.status !== "active") {
        queueLog("info", "connection_queue", job.id, `Skipping job since campaign is no longer active (Status: ${campaignRow ? campaignRow.status : "deleted"}).`);
        continue;
      }
      ...
      if (!isWithinActiveWindow(policy)) {
        ...
        try {
          ...
          queueLog("info", "connection_queue", job.id, `Postponed job to next business hour window (Snoozed until: ${nextWindow}).`);
        } catch (err) {
          queueLog("error", "connection_queue", job.id, `DB update failed during active window snooze: ${err.message}`);
        }
        continue;
      }

      // ── 3. DAILY LIMIT & WARMUP CALCULATOR ─────────────────────────────────
      let limitToday = platformLimits.connections || platformLimits.follows || 15;
      ...
      try {
        ...
        queueLog("error", "connection_queue", job.id, `Failed to query daily_actions count: ${err.message}`);
      }
      ...
      if (todayActionsCount >= limitToday) {
        ...
        try {
          ...
          queueLog("info", "connection_queue", job.id, `Daily outreach rate limit met for campaign (${todayActionsCount}/${limitToday}). Snoozed until tomorrow.`);
        } catch (err) {
          queueLog("error", "connection_queue", job.id, `Failed to record limit_reached event: ${err.message}`);
        }
        continue;
      }
      ...
      try {
        db.prepare("UPDATE connection_jobs SET status = 'running', updated_at = CURRENT_TIMESTAMP WHERE id = ?").run(job.id);
      } catch (err) {
        queueLog("error", "connection_queue", job.id, `Database lock write failed, skipping item to prevent concurrent clashes: ${err.message}`);
        continue;
      }

      report.processed++;
      queueLog("info", "connection_queue", job.id, `Processing connection job for lead ${job.lead_id} (${job.profile_url}).`);
      ...
      try {
        if (res.outcome === "sent") {
          db.transaction(() => {
            ...
            recordCampaignEvent(db, job.campaign_id, job.lead_id, "connection_sent", {
              platform: job.platform,
          ...
          queueLog("info", "connection_queue", job.id, "Successfully sent connection invite.");
          report.success++;
        }
          ...
          db.transaction(() => {
            ...
            // Record Campaign Event
            recordCampaignEvent(db, job.campaign_id, job.lead_id, "connection_skipped", {
          ...
          queueLog("info", "connection_queue", job.id, `Connection skipped / bypassed (Reason: ${res.error || "Already connected"}). Promoted DM.`);
          report.skipped++;
        }
          ...
          queueLog("warn", "connection_queue", job.id, "Platform session validation expired. Postponing job.");
          report.sessionExpired++;
        }
          ...
          queueLog("error", "connection_queue", job.id, `Automation limit or captcha active (Error: ${res.error}). Snoozed.`);
          report.blocked++;
        }
        else {
          ...
          if (newRetryCount >= maxRetries) {
            ...
            db.transaction(() => {
              ...
              recordCampaignEvent(db, job.campaign_id, job.lead_id, "connection_failed_terminal", {
                error: res.error || "Max retries hit"
              });
            })();

            queueLog("error", "connection_queue", job.id, `Terminal invite failure (Cascaded to DM job): ${res.error || "Max retries hit"}`);
            report.failed++;
          } else {
            ...
            recordCampaignEvent(db, job.campaign_id, job.lead_id, "connection_failed_retryable", {
              error: res.error || "Temporary timeout",
              next_attempt: backoffTime
            });

            queueLog("warn", "connection_queue", job.id, `Retryable invite failure: ${res.error}. Scheduled retry at: ${backoffTime}`);
            report.failed++;
          }
        }
      } catch (err) {
        queueLog("error", "connection_queue", job.id, `Failed to persist transaction outcomes in database: ${err.message}`);
      }
      ...
      queueLog("info", "connection_queue", job.id, `Simulating human browser delay: sleeping for ${(randomDelay / 1000).toFixed(1)} seconds.`);
      await sleep(randomDelay);

    } catch (err) {
      queueLog("error", "connection_queue", job.id, `Uncaught isolated item execution exception: ${err.message}`);
    }
  }

  queueLog("info", "connection_queue", "SYSTEM", `Connection queue batch finished: ${JSON.stringify(report)}`);
  return report;
}
