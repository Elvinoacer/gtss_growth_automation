...
async function crawlAndQueueSuggestedAccounts(leadId) {
  ...
  const username = resolveInstagramUsername(lead);

  if (!username) {
    logger.error(
      "IG_DISCOVERY",
      `No Instagram username found for lead ID ${leadId}.`,
    );
  ...
  logger.info(
    "IG_DISCOVERY",
    `Starting suggested accounts crawl for @${username} (Lead ID: ${leadId})`,
  );
  ...
  try {
    ...
    if (toggleBtn) {
      logger.info(
        "IG_DISCOVERY",
        "Clicking Similar accounts toggle button to show suggestions",
      );
      ...
      logger.info(
        "IG_DISCOVERY",
        "Similar accounts toggle button not found or already open.",
      );
    }
    ...
    if (seeAllBtn) {
      logger.info(
        "IG_DISCOVERY",
        "Clicking See all button/link to expand suggestions list",
      );
    ...
    logger.info(
      "IG_DISCOVERY",
      `Found suggested usernames pool: ${pool.join(", ")}`,
    );

    let queuedCount = 0;
    for (const suggestedUser of pool) {
      // Check for duplicates
      const existsInLeads = db
        .prepare(
          "SELECT id FROM leads WHERE ig_username = ? OR profile_url LIKE ?",
        )
        .get(suggestedUser, `%instagram.com/${suggestedUser}%`);
      const existsInQueue = db
        .prepare("SELECT id FROM ig_discovery_queue WHERE ig_username = ?")
        .get(suggestedUser);
      ...
      if (!existsInLeads && !existsInQueue) {
        ...
        logger.info(
          "IG_DISCOVERY",
          `Skipping @${suggestedUser} (already exists in database)`,
        );
      }
    }
    ...
    logger.info(
      "IG_DISCOVERY",
      `Successfully queued ${queuedCount} suggested accounts for @${username}`,
    );
    return { success: true, queuedCount };
  } catch (err) {
    logger.error(
      "IG_DISCOVERY",
      `Error crawling suggested accounts for @${username}: ${err.message}`,
      err,
    );
