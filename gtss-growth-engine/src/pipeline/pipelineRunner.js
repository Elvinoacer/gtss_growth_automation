...
function updatePipelineRun(runId, stageResults) {
  const db = getDb();
  const existing = db.prepare('SELECT stages_json FROM pipeline_runs WHERE id = ?').get(runId);
  let stages = {};
  try {
...
async function sendPipelineSummaryEmail(runId) {
  ...
  const lines = [
    `GTSS Pipeline Run #${runId} — ${run.status.toUpperCase()}`,
  ...
  if (stages.discovery) {
    ...
    lines.push(`Discovery: ${d.newLeads || 0} new leads, ${d.keywordsRun || 0} keywords, ${d.skipped || 0} skipped`);
  }
  if (stages.qualification) {
    const q = stages.qualification;
    lines.push(`Qualification: ${q.qualified || 0} qualified, ${q.deprioritized || 0} deprioritized`);
  }
  if (stages.messages) {
    const m = stages.messages;
    lines.push(`Messages: ${m.generated || 0} generated, ${m.approved || 0} auto-approved`);
  }
  if (stages.send) {
    const s = stages.send;
    lines.push(`Send: ${s.sent || 0} sent, ${s.failed || 0} failed, ${s.skipped || 0} skipped`);
  }

  try {
    await sendNotification(
      `GTSS Pipeline ${run.status === 'completed' ? '✓' : '✗'} Run #${runId}`,
      lines.join('\n'),
    );
  } catch (err) {
    logger.warn('PIPELINE', 'Failed to send summary email', { error: err.message });
  }
}
...
async function runFullPipeline(triggerSource = 'scheduled', options = {}) {
  ...
  const stagesToRun = options.stages || ['discovery', 'qualification', 'messages', 'send'];
  const globalMode = stageMode('discovery'); // reads PIPELINE_MODE

  emit({ type: 'start', message: `Pipeline started (mode: ${globalMode}, trigger: ${triggerSource}, stages: ${stagesToRun.join(', ')})` });
  ...
  try {
    ...
    if (stagesToRun.includes('discovery')) {
      ...
      try {
        ...
        emit({ type: 'stage_done', message: `Discovery: ${result.newLeads} new leads found across ${result.keywordsRun} keywords` });
        updatePipelineRun(pipelineRunId, { discovery: result });
      } catch (err) {
        emit({ type: 'error', message: `Discovery stage failed: ${err.message} — continuing to qualification` });
    ...
    if (stagesToRun.includes('qualification')) {
      ...
      try {
        ...
        emit({ type: 'stage_done', message: `Qualification: ${result.qualified} qualified, ${result.deprioritized} deprioritized` });
        updatePipelineRun(pipelineRunId, { qualification: result });
      } catch (err) {
        emit({ type: 'error', message: `Qualification stage failed: ${err.message} — aborting pipeline` });
    ...
    if (stagesToRun.includes('messages')) {
      ...
      try {
        ...
        emit({ type: 'stage_done', message: `Messages: ${result.generated} generated, ${result.approved} auto-approved (variant ${autoApproveVariant()})` });
        updatePipelineRun(pipelineRunId, { messages: result });
      } catch (err) {
        emit({ type: 'error', message: `Message generation failed: ${err.message} — aborting pipeline` });
        finalisePipelineRun(pipelineRunId, 'failed');
        closePipelineStream(pipelineRunId);
        await sendPipelineSummaryEmail(pipelineRunId);
        return pipelineRunId;
    ...
    if (stagesToRun.includes('send')) {
      ...
      try {
        ...
        emit({ type: 'stage_done', message: `Send: ${result.sent} sent, ${result.failed} failed, ${result.skipped} skipped` });
    ...
    emit({ type: 'complete', message: 'Pipeline finished. See dashboard for full summary.' });
  } catch (err) {
    logger.error('PIPELINE', 'Unhandled pipeline error', { runId: pipelineRunId, error: err.message });
    finalisePipelineRun(pipelineRunId, 'failed');
