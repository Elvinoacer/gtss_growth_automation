...
function cleanupDb(db, leadId) {
  try {
    if (leadId) {
      db.prepare("DELETE FROM ig_warmup_sequences WHERE lead_id = ?").run(leadId);
    ...
    db.prepare("DELETE FROM leads WHERE ig_username = 'test_account_gtss'").run();
...
async function runTests() {
  ...
  try {
    ...
    assert(tables.includes("ig_warmup_sequences"), "Table 'ig_warmup_sequences' is missing.");
    assert(tables.includes("ig_follow_tracker"), "Table 'ig_follow_tracker' is missing.");
    ...
    for (const col of expectedIgColumns) {
      assert(columns.includes(col), `leads table is missing the required Instagram column '${col}'.`);
    }
    ...
    const { filterBusinessProfile } = require("../src/automation/instagramDiscovery");
    ...
    assert.strictEqual(startRes.success, true, `Failed to start sequence: ${startRes.error}`);

    // Verify pending record
    const seqRecord = db.prepare("SELECT * FROM ig_warmup_sequences WHERE lead_id = ?").get(testLeadId);
    assert(seqRecord !== undefined, "Warmup sequence row was not created.");
    assert.strictEqual(seqRecord.status, "pending", `Expected status='pending', got '${seqRecord.status}'`);
    ...
    assert(isDueInList, "Test lead should be flagged as due after next_step_after modification.");

    // Complete warmup
    const completeRes = completeWarmup(testLeadId);
    assert.strictEqual(completeRes.success, true, `Failed to complete warmup: ${completeRes.error}`);
    ...
    `).get(testLeadId);
    assert(msgRecord !== undefined, "Instagram DM draft was not generated in messages table upon sequence completion.");
    ...
    function checkInstagramDailyLimit() {
      const currentFollows = db.prepare(`
    ...
    assert.strictEqual(checkRes.limitReached, true, "Rate limit enforcement should trigger when follows equal configured limits.");
    ...
    const { setInstagramBlockedUntil, isInstagramBlocked } = require("../src/automation/browserBase");

    // Set block
    setInstagramBlockedUntil(24);
    let blockCheck = isInstagramBlocked();
    assert.strictEqual(blockCheck.blocked, true, "Instagram should return blocked: true after triggering blocks.");

    // Clear block
    db.prepare("DELETE FROM settings WHERE key = 'ig_blocked_until'").run();
    blockCheck = isInstagramBlocked();
    assert.strictEqual(blockCheck.blocked, false, "Instagram should return blocked: false after database key cleanup.");
    ...
    // Call POST API
    const postResponse = await fetch(`http://localhost:${TEST_PORT}/api/settings/instagram`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(settingsPayload)
    });
    assert.strictEqual(postResponse.status, 200, `POST Settings failed with status: ${postResponse.status}`);
    const postData = await postResponse.json();
    assert.strictEqual(postData.success, true, "POST Response success state mismatch.");

    // Call GET API
    const getResponse = await fetch(`http://localhost:${TEST_PORT}/api/settings/instagram`);
    assert.strictEqual(getResponse.status, 200, `GET Settings failed with status: ${getResponse.status}`);
    ...
    const pipelineResponse = await fetch(`http://localhost:${TEST_PORT}/api/instagram/warmup-pipeline`);
    assert.strictEqual(pipelineResponse.status, 200, `GET Warmup-pipeline failed with status: ${pipelineResponse.status}`);
    const pipelineData = await pipelineResponse.json();

    assert(pipelineData.stats !== undefined, "Warmup-pipeline shape assertion failed: missing 'stats' key.");
