...
function migrateInstagram() {
  ...
  const migration = db.transaction(() => {
    ...
    const leadsColumns = new Set(
      db.pragma("table_info(leads)").map((c) => c.name),
    );
    ...
    const postsColumns = new Set(
      db.pragma("table_info(posts)").map((c) => c.name),
    );

    const postsMigrations = [
      { col: "ig_post_type", type: "TEXT" },
      { col: "media_paths", type: "TEXT" },
      { col: "location_tag", type: "TEXT" },
    ...
    try {
      db.exec(`
        ...
        WHERE media_paths IS NULL AND media_path IS NOT NULL
      `);
      console.log(
        "✓ posts.media_paths backfilled from posts.media_path where needed",
      );
    } catch (err) {
    ...
    const messagesColumns = new Set(
      db.pragma("table_info(messages)").map((c) => c.name),
    );
  ...
  try {
    migration();
    console.log(
      "[MIGRATE] Instagram database migration successfully completed!",
    );
  } catch (err) {
