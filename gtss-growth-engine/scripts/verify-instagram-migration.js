...
function verifyMigration() {
  ...
  function assertColumnExists(table, column, expectedType) {
    try {
      ...
      if (!col) {
        console.error(
          `❌ [FAIL] Table "${table}" is missing column "${column}"`,
        );
        failedAudits++;
        return false;
      }
      console.log(
        `✓ [PASS] Column "${table}.${column}" exists (Type: ${col.type})`,
      );
      return true;
    } catch (err) {
      console.error(
        `❌ [FAIL] Failed checking columns for "${table}":`,
        err.message,
      );
      failedAudits++;
      return false;
  ...
  function assertTableExists(table) {
    try {
      ...
      console.error(
        `❌ [FAIL] Failed checking existence of table "${table}":`,
        err.message,
      );
      failedAudits++;
      return false;
  ...
  function assertSettingExists(key) {
    try {
      ...
      console.error(
        `❌ [FAIL] Failed checking settings for key "${key}":`,
        err.message,
      );
      failedAudits++;
      return false;
  ...
  try {
    ...
    if (
      ...
      console.log(
        "✓ [PASS] Database structural integrity is perfect (PRAGMA integrity_check: ok)",
      );
    }
  } catch (err) {
  ...
  try {
    ...
    if (fkCheck.length > 0) {
      console.error(
        "❌ [FAIL] Foreign key constraint violations found:",
        fkCheck,
      );
      failedAudits++;
    } else {
      console.log(
        "✓ [PASS] Foreign key constraints are 100% valid (PRAGMA foreign_key_check: no issues)",
      );
    }
  } catch (err) {
  ...
  if (failedAudits > 0) {
    console.error(
      `❌ [AUDIT COMPLETE] Migration verification failed with ${failedAudits} error(s).`,
    );
    process.exit(1);
  } else {
    console.log(
      "✓ [AUDIT COMPLETE] Instagram Migration is 100% robust and correct!",
    );
    console.log("==================================================");
  }
}
